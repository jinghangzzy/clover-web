  ...
  methods: {
    ...
    loginSuccess (res) {
      ...
      */
      ...
      setTimeout(() => {
        this.$notification.success({
          message: '欢迎',
          description: `${this.$store.state.user.loginName}${timeFix()}，欢迎回来`,
          duration: 2
        })
      }, 1000)
    ...
    requestFailed (err) {
      if (err.success === false) {
        ...
        this.$notification['error']({
          message: '登录失败',
          description: err.message || '请求出现错误，请检查网络稍后再试',
          duration: 4
        })
      } else {
        this.$notification['error']({
          message: '网络错误',
          description: '请求失败，请检查网络后再试',
          duration: 4
        })
      }
    }
